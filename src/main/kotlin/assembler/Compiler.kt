...
class Compiler(private val tokens: List<Token>) {
            ...
            for (token in tokens) {
                add(
                    when (token) {
                        ...
                        is Token.Mov -> when (token.register) {
    ...
    private fun getBinaryCode(binOpCode: Int, immediate: Int = 0b0000): UByte {
